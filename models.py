...
False,
description=“True when the requested time is outside the 9am-9pm service window.”
)
date_seems_wrong: bool         = Field(
False,
description=(
“True when the date is logically inconsistent with the intent: “
“a PAST date given for a booking (can’t book retroactively), “
“or a FUTURE date given for a complaint/feedback (incident hasn’t happened yet). “
...
None,
description=“Booking time explicitly mentioned by the user (HH:MM 24hr), if any”
)

```
@field_validator("detected_time", mode="before")
@classmethod
def coerce_detected_time(cls, v):
...
@classmethod
def coerce_detected_date(cls, v):
...
def sync_emergency_flag(self) -> "IntentClassification":
    """Derive is_emergency and date_seems_wrong from detected_date + intent."""
    if self.detected_date is not None:
        delta = (self.detected_date - date.today()).days
        ew = _CONFIG.booking.emergency_window_days
        if 0 <= delta <= ew:
            # Same-day or within emergency window -> emergency booking
            self.is_emergency = True
            self.intent = Intent.EMERGENCY_BOOKING
            self.urgency = UrgencyLevel.CRITICAL
        elif delta > ew and self.intent in (Intent.COMPLAINT, Intent.FEEDBACK):
            # Future date for a complaint/feedback: incident hasn't happened yet
            self.date_seems_wrong = True
        elif delta < 0 and self.intent in (
            Intent.BOOKING_ENQUIRY, Intent.EMERGENCY_BOOKING
        ):
            # Past date for a booking: can't schedule cleaning in the past
            self.date_seems_wrong = True
    # Check service hours from CONFIG
    if self.detected_time is not None:
        h = self.detected_time.hour
        sh = _CONFIG.hours.start_hour
        eh = _CONFIG.hours.end_hour
        if h < sh or h >= eh:
            self.time_outside_hours = True
            # An out-of-hours request is not a real emergency
...
```
# Computed flags -- set by validators, not user input
is_emergency:        bool = False
date_in_past:        bool = Field(
    False,
    description="True when requested_date is before today -- invalid for a booking."
)
time_outside_hours:  bool = Field(
    False,
    description="True when requested_time is outside the 9am-9pm service window."
)
...
def flag_date_issues(self) -> "BookingDetails":
    if self.requested_date is not None:
        ...
        ew = _CONFIG.booking.emergency_window_days
        self.is_emergency = 0 <= delta <= ew
        self.date_in_past = delta < 0
        if self.date_in_past:
            self.requested_date = None
            self.is_emergency = False
    if self.requested_time is not None:
        h = self.requested_time.hour
        sh = _CONFIG.hours.start_hour
        eh = _CONFIG.hours.end_hour
        if h < sh or h >= eh:
            self.time_outside_hours = True
            self.requested_time = None   # clear invalid time, force re-ask
    return self
